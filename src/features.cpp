...
StatefulFeatureDetector* adjusterWrapper(DetectorAdjuster* detadj, int min, int max)
{
  ParameterServer* params = ParameterServer::instance();
  return new  VideoDynamicAdaptedFeatureDetector(detadj, min, max, params->get<int>("adjuster_max_iterations"));
}

StatefulFeatureDetector* adjustedGridWrapper(DetectorAdjuster* detadj, int gridResolution)
{
  ParameterServer* params = ParameterServer::instance();
  int gridcells = gridResolution*gridResolution;
  int max = params->get<int>("min_keypoints");
  ...
  return new VideoGridAdaptedFeatureDetector(detector, max, gridResolution,gridResolution);//Grid raster
}
...
FeatureDetector* createDetector( const string& detectorType ) 
{
    ...
    if( !detectorType.compare( "FAST" ) ) {
        fd = adjustedGridWrapper(new DetectorAdjuster("FAST", 20), 3);
    }
      ...
      /* fd = new SurfFeatureDetector(200.0, 6, 5); */
        ...
        fd = adjustedGridWrapper(new DetectorAdjuster("AORB"),2);
